...
def submit_conversational_answer():
    logger.info("Received audio submission for conversational answer")
    if 'audio' not in request.files:
    ...
    figure = game_session.get_current_figure()
    if not figure:
        logger.error("No current figure available")
        return jsonify({"error": "Game completed"}), 400
    with tempfile.NamedTemporaryFile(delete=False, suffix='.webm') as temp_file:
        audio_file.save(temp_file.name)
        logger.info(f"Saved audio file: {temp_file.name}")
        temp_file_path = temp_file.name
    try:
        wav_path = None
        try:
            ...
            if len(audio) < AUDIO_CONFIG["recording"]["min_duration_ms"]:
                logger.error(f"Audio too short: {len(audio)}ms < {AUDIO_CONFIG['recording']['min_duration_ms']}ms")
                return jsonify({"error": MESSAGES["errors"]["audio_too_short"]}), 400
            wav_path = temp_file_path.replace('.webm', '.wav')
            audio.export(wav_path, format="wav")
            analysis_file_path = wav_path
        ...
        attempt = {
            ...
            "accuracy": "exact" if analysis.get("accuracy_score", 0) >= 90 else
            "close" if analysis.get("accuracy_score", 0) >= 70 else
            "partial" if analysis.get("accuracy_score", 0) >= 40 else "incorrect",
        ...
        game_session.attempts.append(attempt)
        game_session.score += analysis.get("overall_score", 0)
        if analysis.get("accuracy_score", -1) > 0:
            game_session.streak += 1
        else:
            game_session.streak = 0
        ...
        logger.error(f"Error in conversational audio processing error: {e}")
        return jsonify({"error": f"Failed to process audio: {str(e)}"}), 500
    finally:
        cleanup_files = [temp_file_path]
        if 'wav_path' in locals() and wav_path and wav_path != temp_file_path:
...
def calibrate_vad():
    try:
        ...
        logger.info(f"VAD calibrated with baseline: {np.mean(ambient_levels):.3f}, min: {np.min(ambient_levels):.3f}, max: {np.max(ambient_levels):.3f}")
